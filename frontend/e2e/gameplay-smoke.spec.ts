...
test("register, select a character, create a room and start a game", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Powrót do panelu" }).click();
  await page.getByRole("button", { name: "Twórz pokój" }).click();
  ...
  const start = page.getByRole("button", { name: /Start gry/ });
  await expect(start).toBeEnabled();
  await start.click();

  await expect(page.locator(".gameWindow")).toBeVisible();
  await expect(page.getByText("ELDORIA")).toBeVisible();
});
